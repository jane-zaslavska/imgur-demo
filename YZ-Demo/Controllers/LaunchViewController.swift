...
class LaunchViewController: UIViewController {
    ...
    func presentHome() {
        let homeVC = UIStoryboard.main.instantiateViewController(identifier: "HomeViewController")
    ...
    func animateIntro(completion: (()->())? = nil) {
        ...
        UIView.animate(withDuration: 1.5, delay: 0, usingSpringWithDamping: 0.5, initialSpringVelocity: 0, options: .curveEaseOut) {
    ...
    func animateOutro(completion: (()->())? = nil) {
        TapticEngine.vibration.vibrate(effect: .light)
        let newOriginY = calculateNeededOriginY()
        animate(view: stackView, distance: newOriginY, duration: 1.5)
    ...
    private func animate(views: [UIView], distance: CGFloat, duration: TimeInterval) {
        for view in views {
            animate(view: view, distance: distance, duration: 1.5)
        }
    }
